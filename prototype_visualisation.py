...
def visualize_clean_catalog(catalog_path, mode='grid'):
    ...
    for proto_id in sorted(catalog.keys()):
        ...
        u, s, q = entry['source_user'], entry['source_session'], entry['source_seq']
    ...
    print(f"Found {len(catalog)} total prototypes, simplified to {num_unique} UNIQUE prototypes.")
    ...
    if mode == 'grid':
        ...
        else: axes = [axes] # Handle case of 1 subplot
        ...
        plt.title(f"Overlay of {num_unique} Unique Prototype Behaviors (Magnetometer fd_y)", fontsize=14)
        plt.xlabel("Time Step")
        plt.ylabel("Magnetometer fd_y")
        plt.grid(True, alpha=0.3)
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize='small')
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR, f'unique_overlay_epoch_{target_epoch}.png'), dpi=150)
