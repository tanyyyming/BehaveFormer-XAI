...
from model.loss import TripletLoss, PrototypeStructuralLoss
...
def evaluate(
        ...
        for batch_idx, item in enumerate(test_dataloader):
            if imu_type != "none":
                feature_embeddings.append(
                    model([item[0].to(device).float(), item[1].to(device).float()])[0]
                )
            else:
    ...
    if dataname == "humi":
        eer = Metric.cal_user_eer_fixed_sessions(
            torch.cat(feature_embeddings, dim=0).view(
        ...
        eer = Metric.cal_user_eer_vary_sessions(
            torch.cat(feature_embeddings, dim=0),
            num_users,
            user_session_count,
            number_of_enrollment_sessions,
        )[0]
...
def get_args():
    ...
    parser.add_argument(
        "--dataname", help="Dataset name", choices=["humi", "feta"], required=True
    )
    parser.add_argument("-c", "--config", help="Config file", required=True)
    parser.add_argument(
        "--mode",
        choices=["from_scratch", "resume", "transfer_learning"],
        help="Mode",
        default="from_scratch",
    )
    parser.add_argument(
        "--debug", action="store_true", help="Debug mode, only train for 3 epochs"
    )
    parser.add_argument(
        "--work_dirs_subfolder",
        help="Subfolder in work_dirs, used for ablation study to not clutter main results folder",
        default="",
    )
    parser.add_argument(
        "--checkpoint_interval",
        help="Interval to save checkpoint, if >0, checkpoint will be save for based on the interval. Pass -1 to skip",
        default=100,
        type=int,
    )
...
def main(args):
    ...
    work_dir = os.path.join(
        ...
        "work_dirs",
        args.work_dirs_subfolder,
        model_config,
        start_time_fmt,
    )
    if args.debug:
        work_dir = os.path.join(
            os.path.dirname(os.path.dirname(work_dir)), "debug", start_time_fmt
        )
    ...
    ), f"dataname is {dataname} while config's data name is {config_data['dataname']}"
    ...
    if imu_type == "none":
        assert (
            hyperparams["num_imu"] == 0
        ), "Check config file, num_imu must be 0 when imu_type is none"
    else:
        assert (
            len(imu_type.split("_")) == hyperparams["num_imu"]
        ), "Check config file. imu_type and num_imu do not matched"
    imu_feature_dim = hyperparams["num_imu"] * 12
    prototype_projection_epoch_interval = hyperparams[
        "prototype_projection_epoch_interval"
    ]
    number_of_enrollment_sessions = hyperparams["number_of_enrollment_sessions"]
    num_verify_sessions = (
        hyperparams["number_of_verify_sessions"] if dataname == "humi" else None
    )  # If None, determined based on number of sessions for each user
    ...
    if dataname == "humi":
        ...
        val_dataset = HUMITestDataset(
            ...
            validation_file=os.path.join(
                config_data["folders"]["root_dir"],
                config_data["folders"]["data_dir"],
                "validation_scroll_imu_data_all.pickle",
            ),
            imu_type=imu_type,
        )
        # Create dataloaders
    ...
    model = BehaveFormer(
        ...
        target_len,
        gre_k,
        behave_temporal_heads,
        behave_channel_heads,
        imu_temporal_heads,
        imu_channel_heads,
        num_prototypes,
        imu_type=imu_type,
    ...
    structural_loss_fn = PrototypeStructuralLoss()
    ...
    if args.mode == "transfer_learning":  # Load pretrain weights
        ...
        assert (
            init_epoch % 100 == 0
        ), f"Only saved weights every 100 epochs, cannot start from {init_epoch}."
        latest_timestamp = sorted(os.listdir(os.path.dirname(work_dir)))[
            -2
        ]  # the one at -1 is the current output folder
        checkpoint_pt = os.path.join(
            os.path.dirname(work_dir),
            latest_timestamp,
            "checkpoints",
            f"training_{init_epoch}.tar",
        )
